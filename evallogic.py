...
class LogicEvaluator:
	...
	def enter_OrLoOp(self, node):
		
		from visitor import NarrowingData

		target = None

		if isinstance(node.left, astnodes.ULNotOp):
			target = node.left.operand
		
		if isinstance(node.left, astnodes.EqToOp) and isinstance(node.left.right, astnodes.Nil):
			target = node.left.left

		also = None

		if isinstance(self.parent(), astnodes.OrLoOp) and self.parent().left == node:
			also = self.parent().right

		if target:
			self._previous_assumptions[id(node.right)] = target

			if also:
				self._previous_assumptions[id(also)] = target
